...
AWSOME REASON 웹/앱 사이트 프로젝트

![대표 이미지](images/logo.png)
awesome reason.png
## 📋 프로젝트 소개
...
### `/artfinger`
커스텀 테마의 핵심 디렉토리입니다. 모든 레이아웃, 스타일, 스크립트가 이 디렉토리에 포함되어 있습니다.

- **HTML 템플릿**: 헤더, 푸터, 사이드바 등 재사용 가능한 컴포넌트
- **CSS**: 테마 전반의 스타일 정의
- **JavaScript**: 테마별 커스텀 기능 및 유틸리티

### `/product`
상품 관련 모든 페이지와 기능을 포함합니다.

### `/order`
주문 프로세스와 관련된 페이지들을 포함합니다.

### `/myshop`
회원 전용 마이페이지 기능들을 포함합니다.
...
### `artfinger/js/basetool.js`
테마의 기본 유틸리티 및 공통 기능을 담당하는 JavaScript 파일입니다.
- 상품 옵션 선택 기능
- 검색 기능
- 슬라이더 초기화
- 반응형 처리

### `artfinger/css/extend.css`
테마의 확장 스타일을 정의하는 CSS 파일입니다.

### `artfinger/css/main.css`
메인 페이지 스타일을 정의합니다.

### `product/list.js`
상품 목록 페이지의 동적 기능을 처리합니다.

### `product/detail.js`
상품 상세 페이지의 동적 기능을 처리합니다.
...
### 로컬 개발
1. 로컬 웹 서버 실행 (예: Apache, Nginx)
2. 프로젝트 디렉토리를 웹 루트에 배치
3. 브라우저에서 접속하여 확인

### 디버깅
- 브라우저 개발자 도구 활용
- 콘솔 로그 확인 (`console.log` 사용)
- 네트워크 탭에서 리소스 로딩 확인

## 📄 라이선스

이 프로젝트는 Cafe24 전자상거래 플랫폼을 기반으로 합니다.
