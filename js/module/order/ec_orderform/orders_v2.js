...
function createStickyUpdater(elements) {
    ...
    return function updateStickyVariables() {
        ...
        const agreeMsgHeight = isAgreeMsgVisible ? agreeMsgElement.offsetHeight : 0;
        const newStickyBtnTop = newStickyMsgTop + agreeMsgHeight;
...
function createMobilePaddingUpdater(elements) {
    const { checkNoneElement, orderFixItem, agreeMsgElement } = elements;

    return function updateMobilePadding() {
        // 데스크탑: 모든 패딩 제거
        if (window.innerWidth >= 1024) {
            orderFixItem.style.paddingTop = '';
            const mCafe24Order = document.querySelector('.v2 #mCafe24Order');
            if (mCafe24Order) {
                mCafe24Order.style.paddingBottom = '';
            }
            return;
        }

        // 모바일: checkNone 표시 여부 확인
        const isCheckNoneVisible = checkNoneElement &&
            getComputedStyle(checkNoneElement).display === 'block';

        // 주문 버튼 상단 패딩 (checkNone 조건)
        orderFixItem.style.paddingTop = isCheckNoneVisible ? '8px' : '';

        // #mCafe24Order 하단 패딩 계산
        const mCafe24Order = document.querySelector('.v2 #mCafe24Order');
        if (mCafe24Order) {
            let totalHeight = 0;

            // 케이스 1: agree-msg.terms (항상 높이 추가)
            const termsMsg = document.querySelector('.agree-msg.terms');
            if (termsMsg && getComputedStyle(termsMsg).display === 'block') {
                totalHeight += termsMsg.offsetHeight;
            }

            // 케이스 2: agree-msg (terms 제외) - checkNone이 block일 때만
            const generalMsg = document.querySelector('.agree-msg:not(.terms)');
            if (generalMsg &&
                getComputedStyle(generalMsg).display === 'block' &&
                isCheckNoneVisible) {
                totalHeight += generalMsg.offsetHeight;
            }

            // 여백 적용: 문구 높이 + 74px (66 + 8)
            mCafe24Order.style.paddingBottom = totalHeight > 0 ? `${totalHeight + 74}px` : '';
        }
    };
}
