EC$(function () {

    if (EC_MOBILE_DEVICE == true) {
        //실시간 계좌이체, 에스크로 결제 수단 제외 처리
        EC$('#addr_paymethod').find("option[value='tcash']").remove();
        EC$('#addr_paymethod').find("option[value='esc_vcash']").remove();
        EC$('#payment_input_tcash').hide();
        EC$('#payment_input_esc_vcash').hide();
    }

    // 청약 철회 방침에 동의
    EC$('#subscription').on('click', function () {
        if (EC$(this).prop('checked') === true) {
            EC$('#subscription_agreement0').prop('checked', true);
        } else {
            EC$('#subscription_agreement1').prop('checked', true);
        }
    });

    var fixLayerPriceRest = function () {
        EC$('#checked_order_count, #checked_order_price').html('').css('padding-bottom', '0');
    };

    // 고정영역 빈값세팅
    fixLayerPriceRest();

    // 고정영역에 상품정보 세팅
    var fixedLayerPriceSet = function () {
        var iSumPrice = 0;
        var iCheckPrdCnt = 0;
        EC$('[id^="chk_order_cancel_list"]').each(function () {
            if (EC$(this).prop('checked') == true) {
                var sCheckId = EC$(this).attr('id');
                var aTemp = sCheckId.split('_');
                var iCheckId = aTemp[3].replace(/[^0-9]/g, '');
                var iProductPrice = aBasketProductOrderData[iCheckId].product_sum_price;
                iSumPrice = iSumPrice + iProductPrice;
                iCheckPrdCnt = iCheckPrdCnt + 1;
            }
        });
        if (iCheckPrdCnt > 0) {
            var sTotalPrice = CAFE24.SHOP_PRICE_FORMAT.toShopPrice(iSumPrice);
            EC$('#checked_order_count').html('<strong>' + sprintf(__('%s'), iCheckPrdCnt) + '</strong>' + '개 상품선택').css('padding-bottom', '5px');
            EC$('#checked_order_price').html('결제예정금액 <strong><em>' + sTotalPrice + '</em></strong>').css('padding-bottom', '5px');
        } else {
            fixLayerPriceRest();
        }

        var sPriceRef = CAFE24.SHOP_PRICE_FORMAT.shopPriceToSubPrice(iSumPrice);
        if (sPriceRef != '') { EC$('#checked_order_price').find('strong').append(sPriceRef); }
    };

    // 장바구니 체크박스 체크시 상품총합계, 체크한 숫자 구하기
    EC$('[id^="chk_order_cancel_list"]').on('click', function (e) {
        fixedLayerPriceSet();
    });

    // fix주문하기 버튼 클릭
    EC$('#btn_payment_fix').off().on('click', function () {
        EC$('#btn_payment').trigger('click');
    });

    // 상품리스트 전체선택
    EC$('#product_select_all').on('click', function () {
        var _status = EC$(this).data('status');

        EC$('[id^="chk_order_cancel_list"]').each(function () {
            var bChecked = EC$(this).is(":checked");

            if (_status == 'off') {
                if (bChecked === false) { EC$(this).prop('checked', true); }
            } else {
                EC$(this).prop('checked', false);
            }
        });

        EC$(this).data('status', _status == 'off' ? 'on' : 'off');
        fixedLayerPriceSet();
    });

    // 적립금, 마일리지 전체사용
    EC$('#all_use_mileage, #all_use_deposit').off().on('click', function () {
        var id = EC$(this).attr('id');
        var total_mileage = parseInt(CAFE24.SHOP_FRONT_ORDERFORM_DATA.mileage.fMaxMileage);
        var total_deposit = parseInt(CAFE24.SHOP_FRONT_ORDERFORM_DATA.deposit.fMaxDeposit);
        if (id == 'all_use_mileage') {
            EC$('#input_mile').val(total_mileage);
            EC$('#input_mile').trigger('blur');
        } else {
            EC$('#input_deposit').val(total_deposit);
            EC$('#input_deposit').trigger('blur');
        }
    });
});

//정기배송 이용약관 동의
function viewRegularDelivery() {
    var sUrl = '/order/ec_orderform/agreement/regular_delivery.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    window.open(sUrl, '', 'width=640');
}

//쇼핑몰 이용약관 동의
function viewMallAgree() {
    var sUrl = '/order/ec_orderform/agreement/mallagree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//비회원 구매시 개인정보수집이용동의
function viewPersonAgree() {
    var sUrl = '/order/ec_orderform/agreement/personagree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//청약철회방침 보기
function viewSubscription() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/order/ec_orderform/agreement/subscription.html?basket_type=' + EC$('#basket_type').val());
    } else {
        window.open('/order/ec_orderform/agreement/subscription.html?basket_type=' + EC$('#basket_type').val(), '', 'width=640,height=350');
    }
}
//전자보증보험 보기
function viewInsurance() {
    var sUrl = '/order/ec_orderform/agreement/insurance.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//Daum 비회원 구매 동의 보기
function viewDaum() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/order/ec_orderform/agreement/daum.html?basket_type=' + EC$('#basket_type').val());
    } else {
        window.open('/order/ec_orderform/agreement/daum.html?basket_type=' + EC$('#basket_type').val(), '', 'width=640,height=350');
    }
}
//배송정보 제공방침 동의
function viewDelivery() {
    var sUrl = '/order/ec_orderform/agreement/delivery.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//고유식별정보수집 동의
function viewIdentification() {
    var sUrl = '/order/ec_orderform/agreement/identification.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//개인정보수집 이용동의
function viewMemberJoinAgree() {
    var sUrl = '/order/ec_orderform/agreement/privacy_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//개인정보 제3자 제공 동의
function viewInformationAgree() {
    var sUrl = '/order/ec_orderform/agreement/information_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 개인정보 제3자 제공(체크아웃 프로그램) 동의서 (유투브샾스 > 체크아웃)
function viewCheckoutThirdPartyProvideAgree() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/order/agreement/checkout_thirdparty_agreement.html?basket_type=' + $('#basket_type').val() + '&delvtype=' + EC$('#delvType').val());
    } else {
        window.open('/order/agreement/checkout_thirdparty_agreement.html?basket_type=' + $('#basket_type').val() + '&delvtype=' + EC$('#delvType').val(), '', 'width=640,height=350');
    }
}

// 개인정보 국외 이전 동의서 (유투브샾스 > 체크아웃)
function viewCheckoutOverseaTransferAgree() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/order/agreement/checkout_overseas_agreement.html?basket_type=' + $('#basket_type').val() + '&delvtype=' + EC$('#delvType').val());
    } else {
        window.open('/order/agreement/checkout_overseas_agreement.html?basket_type=' + $('#basket_type').val() + '&delvtype=' + EC$('#delvType').val(), '', 'width=640,height=350');
    }
}

//개인정보취급 위탁 동의
function viewConsignmentAgree() {
    var sUrl = '/order/ec_orderform/agreement/consignment_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
//예약결제 이용 동의
function viewCrowdfunding() {
    var sUrl = '/order/ec_orderform/agreement/crowdfunding.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}
function viewSafePhone() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/order/ec_orderform/popup/safePhone.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val());
    } else {
        window.open('/order/ec_orderform/popup/safePhone.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val(), '', 'width=640,height=467');
    }
}

// 결제대행서비스 약관 동의
function viewPaymentAgree() {
    if (EC_MOBILE_DEVICE == true) {
        window.open('/protected/order/payment_agree_financial.html');
    } else {
        window.open('/protected/order/payment_agree_financial.html', '', 'width=640,height=350');
    }
}

// 개인정보 수집 및 이용 동의(선택)
function viewPrivacyOptionalAgree() {
    var sUrl = '/order/ec_orderform/agreement/privacy_optional_agree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 개인정보 수집 및 이용 동의 (주문서 간단 회원가입 시)
function viewPrivacySimpleOrderAgree() {
    var sUrl = '/order/ec_orderform/agreement/privacy_simple_order_agree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 개인정보 제3자 제공 동의
function viewThirdPartyProvideAgree() {
    var sUrl = '/order/ec_orderform/agreement/third_party_provide_agree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 개인정보 처리 위탁 동의
function viewConsignmentProcessingAgree() {
    var sUrl = '/order/ec_orderform/agreement/consignment_processing_agree.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

//마케팅 목적의 개인정보 수집 및 이용 약관
function viewMarketingAgree() {
    var sUrl = '/order/ec_orderform/agreement/marketing_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 사용자 지정 N개 항목 추가 동의 항목 약관
function viewUserDefinedAgree(iAgreementNo) {
    var sUrl = '/order/ec_orderform/agreement/user_defined_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val() + '&agreement_no=' + iAgreementNo;

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

// 예약서비스 상품 구매 동의
function viewReservation() {
    var sUrl = '/order/ec_orderform/agreement/reservation_agreement.html?basket_type=' + EC$('#basket_type').val() + '&delvtype=' + EC$('#delvType').val();

    // URL 에 ch_ref 파라미터 추가
    if (CAFE24.hasChRef()) {
        // 이미 ch_ref가 포함되어 있는지 확인
        var urlObj = new URL(sUrl, window.location.origin);
        if (!urlObj.searchParams.has('ch_ref')) {
            sUrl = CAFE24.attachChRef(sUrl);
        }
    }

    if (EC_MOBILE_DEVICE == true) {
        window.open(sUrl);
    } else {
        window.open(sUrl, '', 'width=640,height=350');
    }
}

//장바구니 선택상품 삭제
function selBasketDel(id) {
    EC$('[id^="' + BASKET_CHK_ID_PREFIX + '"]').prop('checked', false);
    EC$('[id="' + id + '"]').prop('checked', true);
    Basket.deleteBasket();
}

// 도움말 툴팁
EC$('body').on('click', '.mTooltip .eTip', function (e) {
    var findSection = EC$(this).parents('.section').first();
    var findTarget = EC$(EC$(this).siblings('.tooltip'));
    var findTooltip = EC$('.tooltip');
    EC$('.mTooltip').removeClass('show');
    EC$(this).parents('.mTooltip').first().addClass('show');
    EC$('.section').css({ 'zIndex': 0, 'position': 'static' });
    findSection.css({ 'zIndex': 100, 'position': 'relative' });

    findTooltip.hide();
    findTarget.show();
    e.preventDefault();
});
EC$('body').on('click', '.mTooltip .eClose', function (e) {
    var findSection = EC$(this).parents('.section').first();
    var findTarget = EC$(this).parents('.tooltip').first();
    EC$('.mTooltip').removeClass('show');
    findTarget.hide();
    findSection.css({ 'zIndex': 0, 'position': 'static' });
    e.preventDefault();
});
/**
 * 원터치주문서 > 레이어 보기
 */
EC$("body").on("click", ".ec-jigsaw-eLayer", function (e) {
    var findTarget = EC$(EC$(this).attr("href"));
    //call dimmed layer position function
    dimmedLayerPosition(findTarget);
    //흰색 투명 배경 생성.
    findTarget.parent().append("<div id='dimmed_" + findTarget.attr('id') + "' class='dimmed'></div>");
    //흰색 투명 배경이 2개 이상일경우 zindex를 높여서 처리해줌.
    if (EC$(".dimmed").length > 1) {
        EC$(".dimmed").addClass("hide");
        var propZIndex = 110 + EC$(".dimmed").length;
        EC$(findTarget).css({ "zIndex": propZIndex + 5 });
        EC$("#dimmed_" + findTarget.attr("id")).css({ "zIndex": propZIndex }).removeClass("hide");
    }
    e.preventDefault();
});
/**
 * 원터치주문서 > 회색 레이어 위치
 */
function dimmedLayerPosition(target) {
    if (!target.attr("fixed")) {
        var findLayer = target,
            propWinWidth = EC$(window).width(),
            propWinHeight = EC$(window).height(),
            propWidth = findLayer.outerWidth(),
            propHeight = findLayer.outerHeight(),
            propWinScroll = EC$(window).scrollTop();
        if (propWinWidth < propWidth) {
            findLayer.css({ "left": 0, "marginLeft": 0 });
        } else {
            var propLeft = propWidth / 2;
            findLayer.css({ "left": "50%", "marginLeft": "-" + propLeft + "px" });
        }
        var propTop = (propWinHeight / 2) - (propHeight / 2) + propWinScroll;
        findLayer.css({ "top": propTop });
        findLayer.show();
    }
}